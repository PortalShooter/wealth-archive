...
const CompareSide = ({ compareSide, trustVal, willVal, coreVal }) => {
  return (
    <section className={cn('compare-side')}>
      <div className={cn('compare-side__wrapper')}>
        <h2 className={cn('compare-side__title')}>{compareSide.title}</h2>
        <div className={cn('compare-side__columns')}>
          {compareSide.plans.map((item, index) => {
            return index === 0 ? (
              <div className={cn('compare-side__column', 'compare-side__column-desktop')} key={`compare${index}`}>
                <div className={cn('compare-side__column-wrapper')}>
                  <h2 className={cn('compare-side__column-title')}>{compareSide.plans[0].planName}</h2>
                </div>
                {Object.keys(compareSide.comparisonPoints).map((item, index) => {
                  return index === 0 ? null : (
                    <div className={cn('compare-side__block-info')} key={`compareSide${index}`}>
                      ...
                      {index === 1 && (
                        <div className={cn('compare-side__column-info')}>
                          {compareSide.comparisonPoints.wealthPlatformAccess}
                        </div>
                      )}
                      {index === 2 && (
                        <div className={cn('compare-side__column-info')}>
                          {compareSide.comparisonPoints.advancedHealth}
                        </div>
                      )}
                      {index === 3 && (
                        <div className={cn('compare-side__column-info')}>
                          {compareSide.comparisonPoints.durablePower}
                        </div>
                      )}
                      {index === 4 && (
                        <div className={cn('compare-side__column-info')}>
                          {compareSide.comparisonPoints.guardianshipNomination}
                        </div>
                      )}
                      {index === 5 && (
                        <div className={cn('compare-side__column-info')}>{compareSide.comparisonPoints.lastWill}</div>
                      )}
                      {index === 6 && (
                        <div className={cn('compare-side__column-info')}>
                          {compareSide.comparisonPoints.revocableTrust}
                        </div>
                      )}
                    </div>
                  );
              ...
              <div
                className={cn(
                  'compare-side__column',
                  trustVal && index === 3 && 'compare-side__column-active_trust',
                  willVal && index === 2 && 'compare-side__column-active_will',
                  coreVal && index === 1 && 'compare-side__column-active_core'
                )}
                key={`compareColumn${index}`}
              >
                <div
                  className={cn(
                    'compare-side__column-wrapper',
                    trustVal && index === 3 && 'compare-side__column-wrapper_active-trust',
                    willVal && index === 2 && 'compare-side__column-wrapper_active-will',
                    coreVal && index === 1 && 'compare-side__column-wrapper_active-core'
                  )}
                >
                  <h2
                    className={cn(
                      'compare-side__column-title',
                      trustVal && index === 3 && 'compare-side__column-title_active-trust',
                      willVal && index === 2 && 'compare-side__column-title_active-will',
                      coreVal && index === 1 && 'compare-side__column-title_active-core'
                    )}
                  >
                    {item.planName}
                  </h2>
                ...
                <div className={cn('compare-side__column-img')}>
                  {index === 1 && (
                    <div className={cn('compare-side__column-text')}>{compareSide.comparisonPoints.advancedHealth}</div>
                  )}
                ...
                <div className={cn('compare-side__column-img')}>
                  {index === 1 && (
                    <div className={cn('compare-side__column-text')}>{compareSide.comparisonPoints.durablePower}</div>
                  )}
                ...
                <div className={cn('compare-side__column-img')}>
                  {index === 1 && (
                    <div className={cn('compare-side__column-text')}>{compareSide.comparisonPoints.lastWill}</div>
                  )}
                ...
                <div className={cn('compare-side__column-img')}>
                  {index === 1 && (
                    <div className={cn('compare-side__column-text')}>{compareSide.comparisonPoints.revocableTrust}</div>
                  )}
                ...
                {coreVal && index === 1 && (
                  <Link variant="button-fill" className={cn('compare-side__info-btn')} href={compareSide.btnDeskLink}>
                    {compareSide.desktopButtonText}
                  </Link>
                )}
                {willVal && index === 2 && (
                  <Link variant="button-fill" className={cn('compare-side__info-btn')} href={compareSide.btnDeskLink}>
        ...
        <Link variant="button-fill" className={cn('compare-side__btn')} href={compareSide.btnMobileLink}>
          {compareSide.mobileButtonText}
        </Link>
      </div>
    </section>
  );
